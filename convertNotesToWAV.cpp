...
int sampleRate = 8192; // I don't think it's at all my fault that some players
...
int main(int argc, char **argv) {
  ...
  std::fclose(wav);
  if (testCharacter ==
      0x10) // The logic is: if the C library uses big endian for writing binary
            // files, now "testCharacter" will still contain 0.
    isLittleEndian = true;
  else
    isLittleEndian = false;
  wav = std::fopen("output.wav", "wb");
  if (isLittleEndian)
    std::fprintf(wav, "RIFF"); // ASCII for 0x52494646, the magic number that
                               // WAV files start with.
  else
    std::fprintf(wav, "RIFX"); // Big endian WAV file starts with magic number
                               // 0x52494658, or, in ASCII, "RIFX".
  ...
  int32_t ByteRate = 2 * sampleRate; // Since we are using 16 bits per sample,
                                     // and "sampleRate" samples per second.
  ...
  while (!input.eof()) {
    ...
    std::cerr << "Read note name \"" << currentNote
              << "\", the duration string is: " << durationString << std::endl;
    ...
    std::cerr << "Playing note \"" << fullNoteName << "\" for " << noteDuration
              << " samples." << std::endl;
    ...
    for (int i = 0; i < noteDuration; i++) {
      float baseFrequency =
          std::sin(2 * M_PI * currentFrequency * i / sampleRate) *
          std::pow(2, 14);
      float secondHarmony =
          std::sin(2 * M_PI * 2 * currentFrequency * i / sampleRate +
                   M_PI / 4) *
          std::pow(2, 12);
      float thirdHarmony =
          std::sin(2 * M_PI * 3 * currentFrequency * i / sampleRate +
                   M_PI / 2) *
          std::pow(2, 10);
      float fourthHarmony =
          std::sin(2 * M_PI * 4 * currentFrequency * i / sampleRate -
                   M_PI / 4) *
          std::pow(2, 9);
      float currentAmplitude =
          (baseFrequency + secondHarmony + thirdHarmony + fourthHarmony) *
          std::exp(-(float)(2 * i) / (sampleRate)); // Attenuation.
      int16_t numberToBeWritten =
          (fullNoteName == "P") ? (0) : (currentAmplitude);
      numberToBeWritten += std::rand() % (1 << 8) -
                           (1 << 7); // A bit of noise makes it sound better.
